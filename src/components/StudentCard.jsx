...
const StudentCard = ({ name, email, websiteURL, imageURL, gender, skills }) => {
  ...
  return (
    <div className="relative flex w-[350px] h-[220px] md:w-[400px] md:h-[230px] ring-[6px] ring-purple-600 bg-purple-50 shadow-md cursor-pointer rounded-xl overflow-hidden hover:scale-[1.05] hover:shadow-2xl hover:shadow-violet-700 transition duration-300 ease-in">
      ...
      <div className="w-full flex  justify-between px-4 z-10">
        <div className="flex py-10 pl-2 flex-col space-y-1">
          <h2 className="font-inter font-bold text-violet-800 text-3xl capitalize">{name}</h2>
      ...
      <div className="absolute flex justify-center space-x-4 md:space-x-8 bottom-1  rounded-md py-1 px-4  mx-2.5  w-[330px] md:w-[380px] h-[40px]">
        ...
        {skills.map((skill,index) => (
            ...
            <button key={skill.id} className=" z-10 text-violet-700 uppercase  bg-violet-100 hover:bg-violet-600 hover:text-gray-100  font-inter font-bold rounded-md text-xs px-2  text-center transition duration-300 ease-in">
              {skill}
            </button>
